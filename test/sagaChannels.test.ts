...
describe('createNetInfoConnectionChangeChannel', () => {
  ...
  it('netInfoEventChannelFn adheres to eventChannel cb interface', () => {
    ...
    expect(NetInfo.isConnected.addEventListener).toHaveBeenCalledWith('connectionChange', emitMock);

    unsubscribe();
    expect(NetInfo.isConnected.removeEventListener).toHaveBeenCalledWith('connectionChange', emitMock);
  });
});
