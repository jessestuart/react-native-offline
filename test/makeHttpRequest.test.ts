import API, { headers } from 'utils/makeHttpRequest'
import {
  DEFAULT_HTTP_METHOD,
...
global.XMLHttpRequest = class MockXMLHttpRequest {
  public constructor(callbackToFire = '') {
...
describe('API.makeHttpRequest', () => {
  ...
  describe('onload', () => {
    it('resolves the promise if status is 2xx or 3xx', async () => {
      let result = await API.makeHttpRequest({
        ...params,
