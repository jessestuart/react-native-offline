...
import connectivityInterval from '../src/utils/checkConnectivityInterval'
import InternetAccessChecker, {
  checkInternetAccess,
} from '../src/utils/checkInternetAccess'
...
connectivityInterval.setup = jest.fn()
connectivityInterval.clear = jest.fn()
InternetAccessChecker.checkInternetAccess = jest.fn().mockResolvedValue(true)
...
function mockPrototypeMethods(methodsMap: MethodsMap = {}) {
  ...
  Object.entries(methodsMap).forEach(([method, mockFn]: any) => {
    // $FlowFixMe
...
describe('NetworkConnectivity', () => {
  afterEach(() => {
    ...
    connectivityInterval.setup.mockClear()
    connectivityInterval.clear.mockClear()
  })
  ...
  describe('componentDidMount', () => {
    describe('iOS, pingInterval = 0', () => {
      ...
      AND does NOT call setupConnectivityCheckInterval`, () => {
        ...
        expect(connectivityInterval.setup).not.toHaveBeenCalled()
      })
    })
    ...
    it(`calls setupConnectivityCheckInterval with the right arguments
     WHEN pingInterval is higher than 0`, () => {
      ...
      expect(connectivityInterval.setup).toHaveBeenCalled()
    })
  })
  ...
  describe('componentWillUnmount', () => {
    it(`removes the NetInfo listener with the right parameters
      ...
      expect(connectivityInterval.clear).toHaveBeenCalled()
    })
  })
  ...
  describe('checkInternet', () => {
    ...
    with the right arguments if app is in foreground`, async () => {
      ...
      wrapper.instance().handleConnectivityChange = mockHandleConnectivityChange
      await wrapper.instance().checkInternet()
      expect(InternetAccessChecker.checkInternetAccess).toHaveBeenCalledWith({
